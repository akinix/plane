...
<verify>
1. ls yh-flow/src/BuildingBlocks/ 应包含 11 个子目录
2. ls yh-flow/src/Modules/ 应包含 11 个子目录
3. ls yh-flow/src/Host/ 应包含 4 个子目录
4. yh-flow/NuGet.Config 存在
5. yh-flow/src/Directory.Build.props 存在
6. yh-flow/clients/admin/package.json 存在
7. yh-flow/clients/dashboard/package.json 存在
8. yh-flow/clients/admin/src/ 不存在 (已删除)
</verify>
<acceptance_criteria>
...
3. 验证无残留 FSH 引用: grep -r "FSH\." yh-flow/src/ --include="*.cs" --include="*.csproj" | grep -v "obj/" | grep -v "bin/"
4. 如果有残留引用，手动修复
</action>
<verify>
1. yh-flow/src/Host/YH.Flow.Api/ 目录存在
2. yh-flow/src/Host/YH.Flow.AppHost/ 目录存在
3. yh-flow/src/Host/YH.Flow.DbMigrator/ 目录存在
4. yh-flow/src/Host/YH.Flow.Migrations.PostgreSQL/ 目录存在
5. yh-flow/YH.Flow.slnx 存在
6. grep "FSH\." 在 .cs 和 .csproj 文件中无结果 (排除 obj/ 和 bin/)
</verify>
<acceptance_criteria>
...
- 命令 grep -r "FSH\." yh-flow/src/ --include="*.cs" --include="*.csproj" | grep -v "obj/" | grep -v "bin/" 返回空结果
...
<verify>
1. dotnet restore 完成无致命错误
2. dotnet build 输出编译结果 (可能有错误需要 Wave 2 修复)
</verify>
<acceptance_criteria>
