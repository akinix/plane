...
3. 确认编译输出:
   ...
   - 记录警告数量 (应尽可能少)
4. 验证无 FSH 残留:
   - grep -r "FSH\." yh-flow/src/ --include="*.cs" --include="*.csproj" --include="*.json" | grep -v obj | grep -v bin
   - 应为空结果
</action>
<verify>
1. dotnet build 退出码 0
2. 输出包含 "Build succeeded"
3. grep "FSH\." 返回空
</verify>
<acceptance_criteria>
- 命令 dotnet build yh-flow/YH.Flow.slnx 退出码为 0
- 命令 dotnet build yh-flow/YH.Flow.slnx 输出包含 "Build succeeded"
- 命令 grep -r "FSH\." yh-flow/src/ --include="*.cs" --include="*.csproj" --include="*.json" | grep -v obj | grep -v bin 返回空结果 (无 FSH 残留)
...
<verify>
1. Aspire Dashboard 可访问
2. 所有容器在 Dashboard 中显示为 "Running"
3. API 健康检查返回 200
4. pgAdmin, RedisInsight, MinIO Console 端口可访问
</verify>
<acceptance_criteria>
...
<verify>
1. AGENTS.md 不包含 FSH/fullstackhero 的品牌引用 (或已替换为 YH)
2. CLAUDE.md 存在且包含项目基本信息
3. scripts/rename-fsh-to-yh.ps1 存在
4. .editorconfig 存在
</verify>
<acceptance_criteria>
