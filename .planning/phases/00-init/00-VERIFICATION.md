...
| T0.4 | 重命名命名空间 FSH → YH | ✓ VERIFIED | `grep -r "FSH\." --include="*.cs" --include="*.csproj"` 返回 0 结果；命名空间遵循 YH.Framework.*/YH.Modules.*/YH.Flow.* 方案 |
...
| AppHost.cs → MinIO | YH.Flow.Api | Storage__S3__* env vars | ✓ WIRED | AppHost.cs:131-138 |
