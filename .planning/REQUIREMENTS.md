...
- [x] **PAGE-01**: 用户可以看到 Page 列表（含树形层级）
- [x] **PAGE-02**: 用户可以使用 TipTap 编辑器创建/编辑 Page
- [x] **PAGE-03**: 用户可以归档/删除 Page
- [x] **PAGE-04**: 用户可以为 Page 设置访问权限（Public/Private）
- [x] **PAGE-05**: 用户可以收藏/星标 Page

### 视图 (VIEW)

- [x] **VIEW-01**: 用户可以看到已保存的自定义视图列表
- [x] **VIEW-02**: 用户可以将当前筛选/排序/分组配置保存为新视图
- [x] **VIEW-03**: 用户可以应用已保存的视图

### 通知 (NOTI)

- [x] **NOTI-01**: 用户可以看到站内通知列表
- [ ] **NOTI-02**: 用户可以通过 SSE 接收实时通知（自动弹出）
- [x] **NOTI-03**: 用户可以标记通知为已读
- [x] **NOTI-04**: 用户可以点击通知跳转到关联 Issue/评论
