...
public sealed class GenerateTokenCommandHandlerTests
{
    ...
    public async Task Handle_Should_CallAllServicesWithCorrectParameters_When_CredentialsAreValid()
    {
        ...
        await _outboxStore.Received(1).AddAsync(Arg.Any<YH.Framework.Eventing.Abstractions.IIntegrationEvent>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_Should_PassCancellationToken_ToAllServices()
    {
        ...
        await _outboxStore.Received(1).AddAsync(Arg.Any<YH.Framework.Eventing.Abstractions.IIntegrationEvent>(), cancellationToken);
    }
