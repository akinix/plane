...
var postgres = postgresServer.AddDatabase("yhflow-db");
...
builder.AddContainer("redis-insight", "redis/redisinsight", "latest")
    ...
    .WithEnvironment("RI_REDIS_ALIAS0", "yhflow-cache")
...
const string MinioBucket = "yhflow-uploads";
...
var migrator = builder.AddProject<Projects.YH_Flow_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "YH.Flow.Migrations.PostgreSQL")
...
var demoSeeder = builder.AddProject<Projects.YH_Flow_DbMigrator>($"{appPrefix}-demo-seeder")
    .WithReference(postgres)
    .WaitFor(postgres)
...
// API Service
var api = builder.AddProject<Projects.YH_Flow_Api>($"{appPrefix}-api")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WaitFor(redis)
